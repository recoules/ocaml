...
static void frame_descriptors_realloc(
  ...
  caml_frametable_list *new_frametables)
{
  ...
  table->descriptors = (atomic_frame_ptr*)
    caml_stat_calloc_noexc(tblsize, sizeof(atomic_frame_ptr));
...
void caml_init_frame_descriptors(void)
{
  ...
  /* `init_frame_descriptors` is called from `init_gc`, before
     ...
     at will. */
  frame_descriptors_realloc(&current_frame_descrs, frametables);
...
static void stw_frame_descriptors_realloc(
    ...
    caml_domain_state** participating)
{
  ...
  if (caml_global_barrier_is_final(b)) {
    frametable_realloc *request = data;
    ...
    if(tblsize < (table->num_descr + increase) * 2) {
      ...
      frame_descriptors_realloc(table, new_frametables);
    } else {
  ...
  caml_frametable_list *new_frametables)
{
  ...
  if(tblsize < (table->num_descr + increase) * 2) {
    ...
    frametable_realloc request = { table, new_frametables, tail, increase };
    do {} while (!caml_try_run_on_all_domains(
                 &stw_frame_descriptors_realloc, &request, 0));
  } else {
    table->num_descr += increase;
    fill_hashtable(table, new_frametables);
    tail->next = table->frametables;
...
void caml_unregister_frametables(void ** frametables, int ntables)
{
  ...
  for (int i = 0; i < ntables; i++) {
    ...
    for (intnat j = 0; j < len; j++) {
      invalid_entry(&current_frame_descrs, descr);
      descr = next_frame_descr(descr);
    }
    decrease += len;
  }

  current_frame_descrs.num_descr -= decrease;

  previous = &(current_frame_descrs.frametables);

  iter_list(current_frame_descrs.frametables, current) {
  resume:
    for (int i = 0; i < ntables; i++) {
